...
import { createReserve, updateReserve, deleteReserve } from "@/hooks/mutations";
...
export function ReservasView() {
  ...
  const [editingType, setEditingType] = React.useState<"emergency" | "goal" | "investment" | null>(null);
    ...
    <div className={styles.container}>
      <header className={styles.heroHeader}>
        <div className={styles.monthSelector}>
          <button className={styles.navButton} onClick={handlePrevMonth} type="button" aria-label="Mês anterior">
            <ArrowLeft width={20} height={20} />
          </button>
          <h2 className={styles.heroTitle} style={{ fontSize: "2.5rem", margin: 0 }}>
            {monthLabel}
          </h2>
          <button className={styles.navButton} onClick={handleNextMonth} type="button" aria-label="Próximo mês">
      ...
      <Modal open={isModalOpen} onClose={() => setIsModalOpen(false)} title={`Editar ${editingType ? typeLabels[editingType] : ""}`}>
...
function ReserveCard({
  ...
  return (
    <GlassCard className={styles.cardPadding} style={{ position: "relative", minHeight: "220px", display: "flex", flexDirection: "column" }}>
      <div style={{ position: "absolute", top: "1.2rem", right: "1.2rem" }}>
        <button
          onClick={onEdit}
          style={{ background: "rgba(255,255,255,0.05)", border: "none", color: "var(--c-content-muted)", cursor: "pointer", padding: "8px", borderRadius: "50%" }}
          title="Editar"
        >
      ...
      <h4 style={{ fontSize: "1.2rem", fontWeight: 700, marginBottom: "1.5rem", color: "var(--c-content-muted)" }}>{label}</h4>

      <div style={{ marginBottom: "auto" }}>
        <p style={{ fontSize: "2.2rem", fontWeight: 800, color: "var(--c-content)", lineHeight: 1.1 }}>{brl(current)}</p>
        {target > 0 && <p style={{ fontSize: "0.9rem", color: "var(--c-content-muted)", marginTop: "0.4rem" }}>de {brl(target)}</p>}
      </div>
      ...
      {type === "emergency" && runway && (
        <div style={{ marginTop: "1.5rem", padding: "1rem", background: "rgba(255,255,255,0.03)", borderRadius: "12px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            ...
            <span style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--c-action)" }}>{runway} meses</span>
          </div>
          <p style={{ fontSize: "0.7rem", color: "var(--c-content-muted)", marginTop: "0.2rem" }}>base: {brl(essentialAvg || 0)}/mês</p>
        </div>
      )}
      ...
      {type === "goal" && progress !== null && (
        <div style={{ marginTop: "1.5rem" }}>
          ...
          <div style={{ height: "8px", background: "rgba(255,255,255,0.05)", borderRadius: "10px", overflow: "hidden" }}>
            <div style={{ height: "100%", width: progress + "%", background: "var(--status-positive)", borderRadius: "10px" }} />
          </div>
        </div>
      )}
      ...
      {monthly > 0 && (
        <div style={{ marginTop: "1.5rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span style={{ fontSize: "0.8rem", color: "var(--c-content-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Aporte Mensal</span>
